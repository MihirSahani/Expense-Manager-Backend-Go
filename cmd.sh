...
if [ -z "$POSTGRES_SERVER_ADDRESS" ]; then
...
case "$1" in
  "up")
    ...
    if [ -z "$2" ]; then
      migrate -path "$MIGRATION_PATH" -database "$POSTGRES_SERVER_ADDRESS" up
    else
      migrate -path "$MIGRATION_PATH" -database "$POSTGRES_SERVER_ADDRESS" up "$2"
    fi
    ;;
  "down")
    if [ -z "$2" ]; then
      migrate -path "$MIGRATION_PATH" -database "$POSTGRES_SERVER_ADDRESS" down
    else
      migrate -path "$MIGRATION_PATH" -database "$POSTGRES_SERVER_ADDRESS" down "$2"
    fi
